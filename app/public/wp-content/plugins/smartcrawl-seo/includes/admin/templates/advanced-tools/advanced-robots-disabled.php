...
$content         = esc_html__( 'Search engines use web crawlers (bots) to explore and index the internet. A robots.txt file is a critical text file that tells those bots what they can and can’t index, and where things are.', 'smartcrawl-seo' );
	...
	<div class="wds-disabled-component">
		<p>
			<img
				...
				alt="<?php esc_attr_e( 'Disabled', 'smartcrawl-seo' ); ?>" class="wds-disabled-image"/>
		</p>
		...
		<?php if ( ! $already_exists && $rootdir_install ) : ?>
			<input
				...
				value="<?php echo esc_attr__( 'Activate', 'smartcrawl-seo' ); ?>"
				type="submit"/>
		<?php endif; ?>
		...
		if ( $already_exists ) {
			...
			$this->render_view(
				'notice',
				array(
					'message' => esc_html__( "We've detected your site is installed on a sub-directory. Robots.txt files only work when added to the root directory of a domain, so you'll need to change how your WordPress installation is set up to use this feature.", 'smartcrawl-seo' ),
				)
			);
		}
		?>
