	...
	public function get_length_warnings( $title, $description ) {
		...
		if ( $title_length < $title_min_length ) {
			$warnings_markup .= Smartcrawl_Simple_Renderer::load(
				...
				array(
					'message' => sprintf(
						/* translators: 1: Actual length, 2: Minimal length */
						esc_html__( 'Your title is %1$d characters in length. The recommended minimum length is %2$d characters so you may want to consider adding a few words.', 'smartcrawl-seo' ),
						$title_length,
						$title_min_length
					),
				)
		...
		if ( $title_length > $title_max_length ) {
			$warnings_markup .= Smartcrawl_Simple_Renderer::load(
				...
				array(
					'message' => sprintf(
						/* translators: 1: Actual length, 2: Minimal length */
						esc_html__( 'Your title is %1$d characters in length. The recommended max length is %2$d characters so you may want to consider trimming a few words.', 'smartcrawl-seo' ),
						$title_length,
						$title_max_length
					),
				)
		...
		if ( $desc_length < $desc_min_length ) {
			$warnings_markup .= Smartcrawl_Simple_Renderer::load(
				...
				array(
					'message' => sprintf(
						/* translators: 1: Actual length, 2: Recommended minimal length */
						esc_html__( 'Your description is %1$d characters in length. The recommended minimum length is %2$d characters so you may want to consider adding a few words.', 'smartcrawl-seo' ),
						$desc_length,
						$desc_min_length
					),
				)
		...
		if ( $desc_length > $desc_max_length ) {
			$warnings_markup .= Smartcrawl_Simple_Renderer::load(
				...
				array(
					'message' => sprintf(
						/* translators: 1: Actual length, 2: Recommended minimal length */
						esc_html__( 'Your description is %1$d characters in length. The recommended max length is %2$d characters so you may want to consider trimming a few words.', 'smartcrawl-seo' ),
						$desc_length,
						$desc_max_length
					),
				)
	...
	public function smartcrawl_quick_edit_dispatch( $column ) {
		if ( 'smartcrawl-robots' === $column ) {
			Smartcrawl_Simple_Renderer::render(
				...
				array( 'show_title' => Smartcrawl_Settings::get_setting( 'disable-analysis-on-list' ) )
			);
		}
	}
