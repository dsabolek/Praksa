...
(function ($, undefined) {
	...
	if (newsSitemapTab) {
		render(
			<ErrorBoundary>
				<NewsSitemapTab
					newsSitemapUrl={Config_Values.get(
						'news_sitemap_url',
						'news'
					)}
	...
	function update_page_after_report_reload(
		active_issues,
		active_sitemap_issues
	) {
		const $title_issues_indicator = $(
	...
	function add_query_params(params) {
		...
		return (
			current_url.split('?')[0] +
			'?' +
			$.param($.extend({}, { page: current_params.get('page') }, params))
		);
	}
	...
	function init() {
		...
		$(document)
			...
			.on(
				'change',
				'#wds_sitemap_options-sitemap-disable-automatic-regeneration',
				function () {
					const $checkbox = $(this),
						$notice = $checkbox
							.closest('.sui-toggle')
			...
			.on(
				'click',
				'#wds-switch-to-native-sitemap',
				switch_to_native_sitemap
			)
			.on(
				'click',
				'#wds-switch-to-smartcrawl-sitemap',
				switch_to_smartcrawl_sitemap
			)
			.on(
				'click',
				'#wds-deactivate-sitemap-module',
				deactivate_sitemap_module
			)
			.on(
				'click',
				'#wds-manually-update-sitemap',
				manually_update_sitemap
			)
			.on(
				'click',
				'#wds-manually-notify-search-engines',
				manually_notify_search_engines
			);
