...
export default class ExclusionModal extends React.Component {
				...
				/>
				{
					selectedType === 'url' ?
						<TextInputField
							...
							description={__('Enter the URL you want to exclude. Use “*” for wildcard URLs.', 'smartcrawl-seo')}
							errorMessage={__('Please use a relative URL only like /url-to-post.', 'smartcrawl-seo')}
							isValid={isValidUrl}
	...
	getAjaxSearchUrl() {
		...
		return sprintf(
			'%1$s?action=wds-load_exclusion_posts-posts_data-paged&type=%2$s&_wds_nonce=%3$s',
			ajaxUrl,
			this.state.selectedType,
			this.props.nonce
		);
