(function ($) {
	...
	function hook_length_indicators() {
		...
		var desc_min = Wds.get('metabox', 'metadesc_min_length'),
			...
			desc_indicator = new Wds.OptimumLengthIndicator(
				$('#wds_metadesc'),
				replace_macros,
				{
	...
	function hook_analysis() {
		...
		analysis.addEventListener(
			'before-ignore-status-change',
			handle_analysis_ignore_change
		);
		analysis.addEventListener(
			'before-focus-keyword-add',
			handle_focus_keyword_add
		);
		analysis.addEventListener(
			'before-focus-keyword-remove',
			handle_focus_keyword_add
		);
		analysis.addEventListener(
			'before-analysis-refresh',
			handle_analysis_refresh
		);
		analysis.addEventListener(
			'after-seo-analysis-update',
			handle_seo_analysis_update
		);
		analysis.addEventListener(
			'after-readability-analysis-update',
			handle_readability_analysis_update
		);
	}
	...
	function handle_focus_keyword_add() {
		display_navigation_loaders(false);
	}

	function handle_focus_keyword_remove() {
		display_navigation_loaders(false);
	}
	...
	function update_readability_state(event) {
		...
		if (
			$readability_issues.is('.wds-issues-warning') ||
			$readability_issues.is('.wds-issues-error')
		) {
