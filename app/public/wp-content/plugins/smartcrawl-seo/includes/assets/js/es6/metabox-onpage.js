...
class MetaboxOnpage extends EventTarget {
	...
	init() {
		this.editor.addEventListener('autosave', (e) =>
			this.handle_autosave_event(e)
		);
		this.editor.addEventListener('content-change', (e) =>
			this.handle_content_change_event(e)
		);
		...
		$(document)
			.on(
				...
				'.wds-meta-field',
				_.debounce((e) => this.handle_meta_change(e), 200)
			)
			.on(
				'input propertychange',
				'.wds-meta-field',
				_.debounce(
					(e) => this.dispatch_meta_change_deferred_event(e),
					2000
				)
			);

		$(window).on('load', () => this.handle_page_load());
	}
	...
	refresh_preview() {
		...
		let title =
				MetaboxOnpageHelper.get_title() ||
				ConfigValues.get('meta_title', 'metabox'),
			description =
				MetaboxOnpageHelper.get_description() ||
				ConfigValues.get('meta_desc', 'metabox'),
		...
		Promise.all(promises)
			.then((values) => {
				...
				let template = Wds.tpl_compile(
						Wds.template('metabox', 'preview')
					),
					title_max_length = ConfigValues.get(
						'title_max_length',
						'metabox'
					),
					...
					markup = template({
						...
						title: MetaboxOnpage.prepare_string(
							values[0],
							title_max_length
						),
						description: MetaboxOnpage.prepare_string(
							values[1],
							metadesc_max_length
						),
					});
	...
	refresh_placeholders() {
		...
		Promise.all([
			...
			.then((values) => {
				MetaboxOnpageHelper.set_title_placeholder(
					MetaboxOnpage.prepare_string(values[0])
				);
				MetaboxOnpageHelper.set_desc_placeholder(
					MetaboxOnpage.prepare_string(values[1])
				);
			})
			.catch((error) => {
				console.log(error);
