...
class Smartcrawl_Model_Analysis extends Smartcrawl_Model {
	...
	public function update_analysis_data() {
		 ...
		 */
		$checks = Smartcrawl_Checks::apply( $this->post_id, $this->endpoint_remote_handler, $primary_keyword );
		...
		if ( ! empty( $extra_keywords ) ) {
			foreach ( $extra_keywords as $keyword ) {
				$extra_checks = Smartcrawl_Checks::apply( $this->post_id, $this->endpoint_remote_handler, $keyword, false );
				// Add to array.
	...
	public function get_readability_level_description( $level ) {
		...
		/* translators: %s: Level description */
			esc_html__( 'Your content is %s.', 'smartcrawl-seo' ),
			'<strong>%s</strong>'
		);
		...
		$map = array(
			...
			$plain            => sprintf( $content, esc_html__( 'in plain language', 'smartcrawl-seo' ) ),
	 ...
	 *
	 * @return Smartcrawl_Readability_Formula_Flesch
	 */
	private function get_flesch( $string, $lang = '' ) {
		...
		return new Smartcrawl_Readability_Formula_Flesch( $readability_string, $lang );
	}
